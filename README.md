...
-  The **"Relation"** menu offers options for a few types of matching/comparison, based on the search index selected.  Note that "=" is a phrase search within the field, whereas "==" is an exact match of the entire field. Some field may offer the "<>" (not equal) or "empty" options.  Numeric fields like dates may also offer "<" (greater than), ">" (less than), etc.  
