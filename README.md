...
- By default, some text is removed from the field, such as indicators, subfield tags, and the contents of subfield 6 (linkage).  To retain this information in the output, check the option "Include indicators, subfield tags, and linkage in output".  Blank indicators are represented with the underscore _ character.
...
- To retrieve a specific substring from a field, append "(X,Y)" where X is the starting position, and Y is the length.  For example, 008(35,3) retrieves characters 35 through 37 of field 008 (aka the language code).  Character positions are zero-based, (i.e. the first character is in position 0, the second in position 1, etc.).  This is to conform with the MARC specifications for control fields.  Characters can be extracted from the Leader field using "Leader(X,Y)" or "LDR(X,Y)".  If Y is set to 0, then the result will include the entire remaining part of the field starting from position X.
- While the above feature is particularly relevent for fixed fields, it can be used to extract a substring from any other field.  For example, to extract only the indicators from field 245, enter "245(0,1)" and check the "Include indicators..." option.
